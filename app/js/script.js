...
// Carousel
    ...
    // ****** Task #2 : practicing Lodash functions use **** //
    ...
    // data.json
        ...
        // iterate through obj to fetch data needed
        _.each(data, function(person) {
            // aggregate skills as [Array, Array...]
            skills.push(person.skills);
            // aggregate persons + num of friends as [Arr, Arr...] where Arr = [num, person]
            names.push([person.friends.length, person.name]);
            // aggregate friends
            _.each(person.friends, function(obj) {
                friends.push(obj.name);
            });
        });

        // getting array of skills
        // * skills is a nested Array so apply() is needed to use _.union()
        var selectedSkills = _.union.apply(null, skills);
        var sortedSkills = _.sortBy(selectedSkills, function(item) {
                                                        return item.toLowerCase();
                                                    });
        console.log("List of sorted skills, no dublicates:");
        console.log(sortedSkills);

        // getting array of names
        var persons = _.sortBy(names);
        var sortedPersons = _.map(persons, function(p) {
                                              return _.last(p);
                                            });
        console.log("List of persons sorted by the number of friends:");
        console.log(sortedPersons);

        // getting array of friends
        var allFriends = _.union(friends);
        console.log("List of all friends, no dublicates:");
